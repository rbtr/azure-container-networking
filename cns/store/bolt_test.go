...
func openTestNCStore(t *testing.T) store.NCStore {
	t.Helper()
...
func openTestEndpointStore(t *testing.T) store.EndpointStore {
	t.Helper()
...
func TestBoltNCStore_BucketReadWriter(t *testing.T) {
	...
	brw := store.NCStoreBucketReadWriter(s)
	...
	require.NoError(t, brw.Put(bucket, key, val))

	got, err := brw.Get(bucket, key)
	require.NoError(t, err)
	assert.Equal(t, val, got)

	// Delete.
	require.NoError(t, brw.Delete(bucket, key))
	_, err = brw.Get(bucket, key)
	require.Error(t, err)

	// ForEach on an existing bucket should not panic.
	_ = s.PutMeta(ctx, store.StoreMeta{NodeID: "n1"})
	count := 0
	require.NoError(t, brw.ForEach(bucket, func(k, v []byte) error {
		count++
		return nil
	}))
...
func TestBoltEndpointStore_BucketReadWriter(t *testing.T) {
	...
	brw := store.EndpointStoreBucketReadWriter(s)
	require.NotNil(t, brw)

	bucket := []byte("endpoints")
	require.NoError(t, brw.Put(bucket, []byte("raw-key"), []byte("raw-val")))

	v, err := brw.Get(bucket, []byte("raw-key"))
	require.NoError(t, err)
