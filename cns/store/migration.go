...
func MigrateCNSState(ctx context.Context, jsonPath string, dst *NCBoltStore) error {
	if done, err := migrationComplete(dst, migrationKeyCNSStateComplete); err != nil {
...
func MigrateEndpointState(ctx context.Context, jsonPath string, dst *EndpointBoltStore) error {
	if done, err := migrationComplete(dst, migrationKeyEndpointStateComplete); err != nil {
...
// bucketReadWriter is the minimal interface needed by the migration helpers to
// read and write marker keys.  Both *NCBoltStore and *EndpointBoltStore satisfy it.
