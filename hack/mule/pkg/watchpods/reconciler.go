...
func New(nodename string, reconcileFuncs ...reconcile.Func) *PodWatcher { //nolint:revive // private struct to force constructor
	return &PodWatcher{
...
func (p *PodWatcher) Update(pods []v1.Pod) {
	for _, p := range pods {
		fmt.Printf("%s/%s\n", p.Namespace, p.Name)
	}
}
