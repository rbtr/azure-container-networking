...
There is **one PodNetwork per region** — all zones and both test types (rotating + always-on) share the same PN. This is required because DNCRC will not reconcile different PNs with the same network and subnet combination. PodNetworkInstances are namespace-scoped and include a zone suffix (`-z<N>`):

```
BUILD_ID=sv2-long-run-eastus2euap

Shared PodNetwork (one per region, all zones + test types reference this):
  PodNetwork:         pn-longrunning-sv2-long-run-eastus2euap   (VNet: cx_vnet_v1/lr)

Rotating (zone 3 example):
  Namespace:          ns-rotating-z3-sv2-long-run-eastus2euap
  PodNetworkInstance: pni-rotating-z3-sv2-long-run-eastus2euap  → references pn-longrunning-sv2-long-run-eastus2euap
  Deployments:        pod-rotating-0 .. pod-rotating-5          (pod name auto-generated by K8s)

Always-on (DaemonSet, zone 3 example):
  Namespace:          ns-alwayson-z3-sv2-long-run-eastus2euap
  PodNetworkInstance: pni-alwayson-z3-sv2-long-run-eastus2euap  → references pn-longrunning-sv2-long-run-eastus2euap
